...
const TestCard = ({ payments }: { payments: Payment[] }) => {
    return (
        <div className="bg-white rounded-xl overflow-hidden shadow-lg w-full mb-5 p-3">
            <div className="relative">
                ...
                <hr className="mt-4 mb-1 text-neutral-400" />
                    ...
                    <table className="min-w-full text-sm md:text-base text-left text-gray-700">
                        ...
                        <tbody>
                            {payments.map((payment) => (
                                <tr
                                    key={payment.id}
                                    className="block sm:table-row border-b border-neutral-400 sm:border-0 mb-4 sm:mb-0 "
                                >
                                    {/* Invoice */}
                                    <td className="block sm:table-cell px-4 py-2 bg-gray-50 sm:bg-transparent">
                                        <div className="flex sm:block items-start">
                                            <span className="sm:hidden font-semibold w-32 shrink-0">Invoice:</span>
                                    ...
                                    <td className="block sm:table-cell px-4 py-2">
                                        <span className="sm:hidden font-semibold inline-block w-32">Status:</span>
                                        <span
                                    ...
                                    {/* Billing Date */}
                                    <td className="block sm:table-cell px-4 py-2 bg-gray-50 sm:bg-transparent">
                                        <span className="sm:hidden font-semibold inline-block w-32">Billing Date:</span>
                                        {new Date(payment.paymentDate).toLocaleDateString()}
                                    </td>

                                    {/* Amount */}
                                    <td className="block sm:table-cell px-4 py-2">
                                        <span className="sm:hidden font-semibold inline-block w-32">Amount:</span>
                                        ${payment.amountPaid.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                    </td>

                                    {/* Action */}
                                    <td className="block sm:table-cell px-4 py-2 bg-gray-50 mb-1 sm:bg-transparent">
                                        <div className="flex sm:block items-start ">
                                            <span className="sm:hidden font-semibold w-32 shrink-0">Action:</span>
